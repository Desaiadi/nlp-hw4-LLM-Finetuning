...
def create_augmented_dataloader(args, dataset):
    ...
    # Select random 5000 training samples
    train_subset = dataset["train"].shuffle(seed=42).select(range(5000))
    
    # Apply transformation
    transformed_subset = train_subset.map(custom_transform, load_from_cache_file=False)
    
    # Concatenate with original training set
    augmented_dataset = concatenate_datasets([dataset["train"], transformed_subset])
